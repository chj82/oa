...
- `backend/sql/2026072201-system-init.sql` 仅负责创建表、索引等数据库结构。
- 新增数据初始化 SQL，负责写入根部门、默认超级管理员、全部系统接口、系统资源及资源接口关联。
...
新增、删除或修改 Controller 接口路径时，必须同步新增一份位于 `backend/sql/` 的数据变更 SQL，并人工审核执行。接口路径、系统资源和资源接口关联以 SQL 为唯一初始化真相源，不再从运行时代码推导。
